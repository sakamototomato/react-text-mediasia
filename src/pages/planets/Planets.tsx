...
function Planets() {
    ...
    const planets = useMemo(() => data?.map((item) => ({...item, key: item._id, capability: item.minerals})), [data])
    if (isLoading) return <Loading />
  return (
    <div>
        <Table dataSource={planets} columns={columns} pagination={false} footer={undefined} ></Table>
    </div>
  )
}
